...
class NotificationsCell: UITableViewCell {
    ...
    override func awakeFromNib(){
        ...
        borderTop.frame = CGRect(x: 0, y: 0, width: (self.frame.size.width), height: 1)
        borderTop.borderWidth = 1
        let borderBottom = CALayer()
        borderBottom.borderColor = UIColor(red:0.20, green:0.67, blue:0.65, alpha:0.3).cgColor
        borderBottom.frame = CGRect(x: 0, y: (self.viewSwitch.frame.size.height) - 1, width: (self.frame.size.width), height: 1)
